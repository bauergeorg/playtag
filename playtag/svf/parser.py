...
class ParseSVF(object):
    ...
    nodata = DataStream()
    states = AnyDict()
    for (x,y) in vars(jtagstates).items():
        ...
        for param in iterparams:
            ...
            if num is not None:
                ...
                if do_end and not did_end and clock_specified:
                    self.ENDSTATE = self.states[param]
                    did_end = True
                elif not do_end and not clock_specified:
                    self.RUNSTATE = self.ENDSTATE = self.states[param]
                else:
    ...
    def parse(self, fname):
        ...
        try:
            for params, linenum in myiter:
                ...
                cmd = next(iterparams) # python 2: iterparams.next()
                print(cmd)
                cmdproc = getproc(cmd)
                if cmdproc is None:
            ...
            raise SvfError("Error in line {} of file {}:\n   Command {}".format(linenum, fname, cmd))
